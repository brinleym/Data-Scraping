...
class Query:
	...
	def validateSeed(self):
		...
		if(response.status_code == requests.codes.ok):
			print "Seed valid!"
			print("Starting to crawl " + self.maxPages + " webpages.")
			self.initiateCrawler(self.seed, self.maxPages)
			return True
		else:
	...
	def initiateCrawler(self, seed, maxPages):

		myCrawler = Crawler(seed, maxPages) # instantiate Crawler
		self.results = myCrawler.crawl() # start crawling
		self.printResults(self.results) # keep this for debugging

		myResults = Results(self.query, self.seed) # instantiate Results object
		myResults.getResults(self.results)
		myResults.printResults()
		myResults.writeResults()
