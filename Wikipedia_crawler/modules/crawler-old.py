...
#finds and stores links on a webpage in list structure
class Parser:
	...
	def makeRequest(self):
		try:
			...
			print error
			print "Error occured. Page not found. Returning false.."
			return False
...
class Query:
	...
	def initiateCrawler(self, seed, maxPages):
		...
		#if(self.seedValid):
			#if seed is valid url, start crawling..
		myCrawler = Crawler(seed, maxPages)
		self.results = myCrawler.crawl()
		self.printResults(self.results)
		#else:
			#print "There is no valid wikipedia page for your topic."
